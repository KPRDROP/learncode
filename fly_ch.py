...
# Fixed Referer and origin
REFERER = "https://flyembed.click/"
...
async def get_events(cached_keys: Set[str]) -> list[Event]:
    ...
    # Fix: Use Time.rn() instead of Time.clean(Time.now())
    now = Time.rn()
...
def generate_vlc_playlist(data: dict[str, dict]) -> int:
    ...
    lines = ["#EXTM3U"]
    lines.append(f"# Playlist generated by {TAG} - {Time.rn().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append("")
    count = 0
    chno = 1
    ...
    for name, entry in sorted(data.items()):
        ...
        lines.append(f'#EXTINF:-1 tvg-chno="{chno}" tvg-id="{tvg_id}" tvg-logo="{tvg_logo}" group-title="{group_title}",{display_name}')
...
def generate_tivimate_playlist(data: dict[str, dict]) -> int:
    ...
    for name, entry in sorted(data.items()):
        ...
        lines.append(f'#EXTINF:-1 tvg-chno="{chno}" tvg-id="{tvg_id}" tvg-logo="{tvg_logo}" group-title="{group_title}",{display_name}')
        lines.append(f"{url}|referer={referer}|origin={referer}|user-agent={encoded_user_agent}")
        lines.append("")
        count += 1
        chno += 1
...
async def scrape() -> None:
    ...
    if valid_events:
        ...
        with open(OUTPUT_TIVIMATE, "w") as f:
            f.write("#EXTM3U\n# No events available\n")
        OUTPUT_VLC.chmod(0o644)
        OUTPUT_TIVIMATE.chmod(0o644)
